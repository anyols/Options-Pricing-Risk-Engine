...
from optrisk.instruments.option import MarketEnv, Model, OptionSpec, Stock
...
class Position:
    ...
    def price(self, model: Model = "auto", **model_kwargs: int) -> float:
    ...
    def greeks(self, model: Model = "auto", **model_kwargs: int) -> Greeks:
        if isinstance(self.instrument, Stock):
            return Greeks(delta=1.0)
        return self.instrument.greeks(self.market, model=model, **model_kwargs)

    def value(self, model: Model = "auto", **model_kwargs: int) -> float:
        return self.quantity * self.multiplier * self.price(model=model, **model_kwargs)

    def position_greeks(self, model: Model = "auto", **model_kwargs: int) -> Greeks:
