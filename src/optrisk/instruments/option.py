...
__all__ = ["MarketEnv", "OptionSpec", "Stock"]
...
class OptionSpec:
    ...
    def price(
        ...
        *,
        ...
        if resolved == "bsm":
            return bsm_price(market.spot, self.strike, market.rate, market.dividend_yield, market.vol, self.expiry, self.option_type)
        if resolved == "black76":
            return black76_price(market.spot, self.strike, market.rate, market.vol, self.expiry, self.option_type)
        if resolved == "binomial":
            return crr_price(
                market.spot, self.strike, market.rate, self._carry_yield(market), market.vol, self.expiry,
                self.option_type, style=self.style, steps=steps,
            )
        if resolved == "monte_carlo":
            ...
            return mc_price(
                market.spot, self.strike, market.rate, self._carry_yield(market), market.vol, self.expiry,
                self.option_type, n_paths=mc_paths, seed=mc_seed,
            ).price
    ...
    def greeks(
        ...
        *,
        ...
        resolved = self._resolve_model(model)
        if resolved == "bsm":
            return bsm_full_greeks(market.spot, self.strike, market.rate, market.dividend_yield, market.vol, self.expiry, self.option_type)
        if resolved == "black76":
        ...
        if resolved in ("binomial", "monte_carlo"):
            ...
            def pricer(spot: float, vol: float, expiry: float, rate: float) -> float:
                ...
                return bumped_instrument.price(bumped_market, model=resolved, steps=steps, mc_paths=mc_paths, mc_seed=mc_seed)
