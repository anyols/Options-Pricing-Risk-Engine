...
from optrisk.viz.theme import AMBER, GREEN, NAVY, RED, TEAL
...
def _init_state() -> None:
    ...
    if "custom_market" not in st.session_state:
        st.session_state.custom_market = dict(spot=100.0, rate=0.04, vol=0.25, dividend_yield=0.01)
    ...
    for leg in st.session_state.custom_positions:
        if leg["kind"] == "Stock":
            ...
            instrument = OptionSpec(
                ...
                style=leg["style"], underlying_type="equity",
            )
        portfolio.add(Position(instrument=instrument, quantity=leg["quantity"], market=market, label=leg["label"], multiplier=leg.get("multiplier", 100)))
    return portfolio
...
def sidebar_portfolio() -> Portfolio:
    ...
    if mode == "Demo multi-underlying book":
        st.sidebar.info("A 6-leg book across a fictitious equity (TECH) and a futures option (CRUDE), with a partial delta hedge.", icon="\U0001F4BC")
    ...
    m["dividend_yield"] = st.sidebar.number_input("Dividend yield", value=m["dividend_yield"], format="%.4f", step=0.005)
    ...
    with st.sidebar.form("add_leg", clear_on_submit=True):
        ...
        if submitted:
            if kind == "Stock":
                ...
                leg = {
                    "kind": "Option", "option_type": opt_type, "strike": strike, "expiry": expiry,
                    "style": style, "quantity": quantity, "multiplier": 100,
...
st.caption("Portfolio pricing, Greeks, scenario analysis and delta-hedging on Black-Scholes / Black-76 / binomial / Monte Carlo / Heston")

if len(portfolio) == 0:
    st.stop()

tab_summary, tab_greeks, tab_scenario, tab_hedge, tab_smile = st.tabs(
    ["\U0001F4CB Portfolio Summary", "\U0001F4D0 Greeks Explorer", "\U0001F30B Scenario Analysis", "⚖️ Delta-Hedging Simulator", "\U0001F30A Vol Smile (Heston)"]
)
...
with tab_summary:
    ...
    with left:
        ...
        display_cols = ["label", "quantity", "multiplier", "spot", "vol", "price", "value", "delta", "gamma", "vega", "theta", "rho"]
        st.dataframe(frame[display_cols].style.format({c: "{:,.2f}" for c in display_cols if c not in ("label", "multiplier")}), width="stretch", hide_index=True)
    with right:
...
with tab_greeks:
    ...
    greek_choice = st.multiselect("Greeks to plot", list(greek_frame.columns), default=["delta", "gamma", "vega", "theta"])
    if greek_choice:
        ...
        for i, (g, color) in enumerate(zip(greek_choice, PALETTE), start=1):
            fig.add_trace(go.Scatter(x=greek_frame.index, y=greek_frame[g], name=g, line=dict(color=color, width=2.5)), row=1, col=i)
...
with tab_scenario:
    ...
    st.caption("The gap between the exact repriced P&L and each Taylor order is the direct measure of nonlinear portfolio risk.")
    ...
    with surf_col:
        ...
        order = None if surface_choice == "Full reprice" else TAYLOR_ORDERS[[TAYLOR_ORDER_LABELS[o] for o in TAYLOR_ORDERS].index(surface_choice)]
    ...
    with slice_col:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=spot_shocks * 100, y=result.full_pnl[vol_idx], name="Full reprice", line=dict(color=NAVY, width=3)))
        for o, color, dash in zip(TAYLOR_ORDERS, [_R, _A, _T, _P], ["dash", "dashdot", "dot", "dash"]):
            fig.add_trace(go.Scatter(x=spot_shocks * 100, y=result.taylor_pnl[o][vol_idx], name=TAYLOR_ORDER_LABELS[o], line=dict(color=color, width=1.8, dash=dash)))
        fig.update_layout(template=plotly_template(), height=420, xaxis_title="Spot shock (%)", yaxis_title="P&L ($)", legend=dict(orientation="h", y=-0.25))
        st.plotly_chart(fig, width="stretch")

    st.subheader("Approximation Error by Taylor Order")
    error_cols = st.columns(len(TAYLOR_ORDERS))
    for col, order_name in zip(error_cols, TAYLOR_ORDERS):
        col.metric(TAYLOR_ORDER_LABELS[order_name], f"${result.max_abs_error(order_name):,.0f}", help="Max |Taylor approx - full reprice| over the grid")
...
with tab_hedge:
    ...
    st.caption("Delta-hedge an option through expiry under a *realized* vol that can differ from the *implied* vol used to hedge.")
    ...
    single_path = simulate_delta_hedge(
        spot0=h_spot, strike=h_strike, rate=h_rate, dividend_yield=0.0, implied_vol=h_iv, realized_vol=h_rv,
        expiry=h_expiry, option_type=h_type, option_quantity=qty_val, n_steps=252, seed=seed,
    )
    ...
    with st.spinner("Running hedge simulations across rebalancing frequencies..."):
        freq_frame = run_hedge_frequency_comparison(
            spot0=h_spot, strike=h_strike, rate=h_rate, dividend_yield=0.0, implied_vol=h_iv, realized_vol=h_rv,
            expiry=h_expiry, option_type=h_type, option_quantity=qty_val,
            frequencies={"Daily": 252, "Weekly": 36, "Monthly": 12}, n_paths=n_paths, seed=seed,
        )
    ...
    with stat_col:
        ...
        st.info(f"Position gamma is **{gamma_sign}**, and realized vol **{vol_gap}** implied vol -- theory predicts the sign of the average hedging P&L above.", icon="\U0001F4A1")
    with chart_col:
...
with tab_smile:
    ...
    with st.spinner("Pricing across strikes..."):
        heston_ivs = heston_implied_vol_smile(
            spot=s_spot, rate=s_rate, dividend_yield=0.0, expiry=s_expiry,
            v0=s_v0, kappa=s_kappa, theta=s_theta, xi=s_xi, rho=s_rho, strikes=strikes, option_type="call",
        )
    flat = np.full_like(strikes, np.sqrt(s_theta))
    st.plotly_chart(plotly_vol_smile(strikes, {"Heston (stochastic vol)": heston_ivs, "Flat BSM assumption": flat}, spot=s_spot), width="stretch")
